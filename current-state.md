...
- _**OFF / follow-ups:** auto-send confirmations is gated by `AUTO_SEND_CONFIRMATIONS` (unset = off; confirmations still queue as drafts). The **Bulk Send tab now has an editable recipient list** — load a group (program/week/day/all), then remove any address (× chip) or add more; the chips are the source of truth and fan out to individual private sends. A dedicated Compose screen is still an optional follow-up. Also pending: a one-off to send the 13 stranded draft confirmations._
