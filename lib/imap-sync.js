    ...
    try {
      ...
      for await (const msg of client.fetch(batch.join(','), { uid: true, source: true, internalDate: true }, { uid: true })) {
        try {
          ...
          // A single malformed/oversized message must not abort the batch and stall
          // every newer message behind it. Log it and write a deletedAt-marked
          // tombstone carrying this uid, so the cursor advances past it (getInboundState
          // counts deleted rows) and it isn't retried. The tombstone never shows in the
          // inbox. If even the tombstone write fails, this uid is retried next run.
          console.error(`IMAP sync ${account.key}: skipping uid ${msg.uid}: ${e.message}`);
          const synthId = `imap-${account.key}-${uidValidity}-${msg.uid}`;
          try {
            const nowTs = new Date().toISOString();
            await db.createInboundEmail({
              id: inboundId(account.key, synthId),
              direction: 'in', mailbox: account.key,
              messageId: synthId, imapUid: msg.uid, imapUidValidity: parseInt(uidValidity, 10),
              subject: '(unreadable message)', status: 'sync_failed',
              syncError: (e.message || '').slice(0, 300),
              read: true, deletedAt: nowTs, deletedBy: 'sync',
              receivedAt: nowTs, createdAt: nowTs,
            });
            state.messageIds.add(synthId);
          } catch (_) { /* tombstone write failed; uid retried next run */ }
          skipped++;
        }
      }
      return { mailbox: account.key, stored, skipped, pending };
    } finally {
      lock.release();
    }
  } finally {
...
async function syncAllMailboxes() {
  ...
  return { configured: true, total, skipped, results };
}
