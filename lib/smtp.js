...
async function sendReply(opts) {
  // H4 fix: only fall back to the first configured mailbox when NO accountKey was
  // requested. If a specific mailbox was asked for but isn't configured, throw a
  // clear error instead of silently sending an info@ thread as registration@.
  const account = opts.accountKey
    ? mailConfig.getAccount(opts.accountKey)
    : mailConfig.configuredAccounts()[0];
  if (!account) {
    throw new Error(opts.accountKey
      ? `No mailbox configured for "${opts.accountKey}".`
      : 'No mailbox configured for replies.');
  }
